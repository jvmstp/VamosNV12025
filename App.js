...
const App = () => {
  ...
  const addSupplier = useCallback(() => {
    try {
      if (name && address && contact && category) {
        setSuppliers(currentSuppliers => [
          ...currentSuppliers,
          {
            ...
            contact,
            category,
            image: image || 'https://reactnative.dev/img/tiny_logo.png',
          },
        ]);
        setName('');
        setAddress('');
      ...
      Alert.alert('Erro', 'Não foi possível adicionar o fornecedor. Tente novamente.');
      console.log('Erro ao adicionar fornecedor:', error);
    }
  }, [name, address, contact, category, image]);
